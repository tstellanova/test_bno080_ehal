...
use embedded_hal::digital::v2::{toggleable, OutputPin, StatefulOutputPin, ToggleableOutputPin};
...
impl<T> toggleable::Default for ToggleThing<T> where T: StatefulOutputPin + OutputPin {}

pub fn setup_peripherals() -> (UserLed1Type, impl DelayMs<u8>, ImuI2cPortType, BnoSpi1Lines) {
    ...
    let spi_ctrl_lines = {
        ...
        // SPI chip select CS
        let csn = port0.p0_25.into_push_pull_output(Level::High);

        // HINTN interrupt pin
        let hintn = port0.p0_13.into_floating_input().degrade();

        // WAKEN pin / PS0
        let waken = port0.p0_18.into_push_pull_output(Level::High);

        // NRSTN pin
        let reset_pin = port0.p0_26.into_push_pull_output(Level::High);
